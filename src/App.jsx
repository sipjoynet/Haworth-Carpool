...
function LoginScreen() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // If auth fails, check if this is an old user with plain-text password
      if (authResult.error) {
        console.log('Supabase Auth login failed, checking for legacy user...');
        ...
        // Check if user exists in database with plain-text password
        const dbUser = await DB.findOne('users', u => u.email === email);
        ...
        // Case 1: User already linked to Auth but password doesn't work
        if (dbUser && dbUser.auth_user_id && !dbUser.password) {
          console.log('⚠️ User already migrated to Supabase Auth');
          setError('This account uses Google sign-in or has been migrated. Please use "Sign in with Google" or reset your password.');
          return;
        }
        ...
        if (dbUser && dbUser.password === password && password.length >= 6 && !dbUser.auth_user_id) {
          ...
          if (signupError) {
            ...
            setError('Error migrating account. Please contact admin.');
            return;
          }
          ...
          await DB.update('users', dbUser.id, {
          ...
          // Use the session from signup (don't need to sign in again)
          // If there's a session, use it; otherwise the user needs to confirm their email
          if (signupData.session) {
            authResult = { data: signupData, error: null };
          } else {
            // Email confirmation required
            setError('Account migrated! Please check your email to confirm your account, then try logging in again.');
            return;
          }
        } else {
      ...
      const user = await DB.findOne('users', u => u.auth_user_id === authResult.data.user.id);
